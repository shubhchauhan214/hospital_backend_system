...
def create_doctor_availability(availability: schemas.DoctorAvailabilityCreate, db: Session = Depends(get_db)):
...
def get_doctor_availabilities(skip:int = 0, limit:int = 100, db: Session = Depends(get_db)):
    return crud.get_doctor_availibilities(db=db, skip=skip, limit=limit)
...
def update_doctor_availability(availability_id:int, availability:schemas.DoctorAvailabilityUpdate, db:Session=Depends(get_db)):
    return crud.update_doctor_availability(db=db, availability_id=availability_id, availability_data=availability)

@router.delete("/{availability_id}")
def delete_doctor_availability(availability_id:int, db:Session=Depends(get_db)):
