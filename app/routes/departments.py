...
def create_department(department: schemas.DepartmentCreate, db: Session = Depends(get_db), current_user=Depends(admin_required)):
...
def update_department(department_id: int, department: schemas.DepartmentUpdate, db: Session = Depends(get_db), current_user=Depends(admin_required)):
    return crud.update_department(db=db, department_id = department_id, department_data =department)


@router.delete("/{department_id}")
def delete_department(department_id: int, db: Session=Depends(get_db), current_user=Depends(admin_required)):
